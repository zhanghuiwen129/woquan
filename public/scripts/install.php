...
// 定义安装状态常量
const INSTALL_LOCK_FILE = '../install/install.lock';
    ...
    function env($key, $default = null) {
        ...
        foreach ($lines as $line) {
            ...
            if (trim($envKey) === $key) {
                $value = trim($envValue);
                // 调试：记录读取的配置
                error_log("env() 读取配置: $key = " . ($key === 'DATABASE_PASSWORD' ? '***' : $value));
                return $value;
            }
        }
...
function checkInstallationStatus() {
    $installed = file_exists(INSTALL_LOCK_FILE);
...
switch ($step) {
    case 0:
        // 免责声明
        include '../install/disclaimer.php';
        break;
    case 1:
        // 环境检测
        include '../install/env_check.php';
        break;
    case 2:
        // 数据库配置
        include '../install/db_config.php';
        break;
    case 3:
        // 数据库安装
        include '../install/db_install.php';
        break;
    case 4:
        // 管理员设置
        include '../install/admin_setup.php';
        break;
    case 5:
        // 安装完成
