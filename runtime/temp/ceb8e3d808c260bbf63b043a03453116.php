<?php /*a:2:{s:36:"D:\wwwroot\view\index\favorites.html";i:1770122626;s:38:"D:\wwwroot\view\index\main-layout.html";i:1770774785;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
<div id="favorites-list" class="space-y-4">
    <?php if(is_array($favorites) || $favorites instanceof \think\Collection || $favorites instanceof \think\Paginator): $i = 0; $__LIST__ = $favorites;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
    <div class="favorite-item">
        <a href="/card/<?php echo htmlentities((string) $item['user_id']); ?>" class="flex items-center gap-3">
            <img src="<?php echo htmlentities((string) (isset($item['avatar']) && ($item['avatar'] !== '')?$item['avatar']:'/static/images/default-avatar.png')); ?>" alt="头像" class="favorite-avatar">
            <div class="flex-1">
                <h3 class="font-semibold text-gray-800"><?php echo htmlentities((string) (isset($item['nickname']) && ($item['nickname'] !== '')?$item['nickname']:$item['username'])); ?></h3>
                <p class="text-sm text-gray-500 truncate"><?php echo htmlentities((string) (isset($item['bio']) && ($item['bio'] !== '')?$item['bio']:'暂无个性签名')); ?></p>
            </div>
        </a>
    </div>
...
<div id="empty" class="text-center py-16">
    ...
    <h2 class="text-xl font-bold text-gray-800 mb-2">暂无收藏</h2>
    <p class="text-gray-500 mb-4">快去发现有趣的内容吧</p>
    <a href="/discover" class="inline-block bg-primary text-white px-6 py-2 rounded-full hover:bg-secondary transition-colors">
        去发现
    </a>
</div>
<?php endif; ?>
...
<!-- 收藏列表 - 使用PHP渲染的数据 -->
<?php if(isset($favorites) && count($favorites) > 0): ?>
<div id="mobile-favorites-list" class="space-y-3">
    <?php if(is_array($favorites) || $favorites instanceof \think\Collection || $favorites instanceof \think\Paginator): $i = 0; $__LIST__ = $favorites;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
    <div class="favorite-item">
        <a href="/card/<?php echo htmlentities((string) $item['user_id']); ?>" class="flex items-center gap-3">
            <img src="<?php echo htmlentities((string) (isset($item['avatar']) && ($item['avatar'] !== '')?$item['avatar']:'/static/images/default-avatar.png')); ?>" alt="头像" class="favorite-avatar">
            <div class="flex-1 min-w-0">
                <h3 class="font-semibold text-gray-800 text-sm truncate"><?php echo htmlentities((string) (isset($item['nickname']) && ($item['nickname'] !== '')?$item['nickname']:$item['username'])); ?></h3>
                <p class="text-xs text-gray-500 truncate"><?php echo htmlentities((string) (isset($item['bio']) && ($item['bio'] !== '')?$item['bio']:'暂无个性签名')); ?></p>
            </div>
        </a>
    </div>
    <?php endforeach; endif; else: echo "" ;endif; ?>
...
<div id="mobile-empty" class="text-center py-16">
    ...
    <h2 class="text-lg font-bold text-gray-800 mb-2">暂无收藏</h2>
    <p class="text-gray-500 text-sm mb-4">快去发现有趣的内容吧</p>
    <a href="/discover" class="inline-block bg-primary text-white px-5 py-2 rounded-full hover:bg-secondary transition-colors text-sm">
        去发现
    </a>
</div>
<?php endif; ?>
